...
export default function MensajesAdminScreen() {
  ...
  const renderItem = ({ item }) => (
    <View style={styles.row}>
      ...
      <Text style={styles.muted}>{item.enviado_por_nombre} · {new Date(item.created_at).toLocaleString('es-AR')}</Text>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>No hay mensajes enviados.</Text>}
      />
    </View>
  );
}
